...
class Controller:
    ...
    def readConfig(self):
        try:
            ...
            trackSettings.comPort = int(clines[1])
            trackSettings.camera = int(clines[2])
    ...
    def writeConfig(self):
        ...
        config.write(str(trackSettings.comPort) + '\n')
        config.write(str(trackSettings.camera) + '\n')
        ...
        config.write(str(trackSettings.Lat) + '\n')
        config.write(str(trackSettings.Lon) + '\n')
    ...
    def separation_between_coordinates(self):
        self.separation = math.degrees(math.acos(
            math.sin(self.Y1) * math.sin(self.Y2) + math.cos(self.Y1) * math.cos(self.Y2) * math.cos(
                self.X1 - self.X2)))
        ...
        if trackSettings.telescopeType == 'ASCOM':
            if trackSettings.mountType == 'AltAz':
                ...
                if starty < (self.height / 2):
                    ...
                    image_scale = self.separation / distmoved
                else:
                    distmoved = 0
                    self.tel.MoveAxis(1, -0.1)
                    while distmoved < 100 and trackSettings.calibratestart is True:
            ...
            if trackSettings.mountType == 'Eq':
                ...
                if starty < (self.height / 2):
                    ...
                    return self.separation / distmoved
                else:
                    distmoved = 0
                    self.tel.MoveAxis(1, -0.1)
                    while distmoved < 100 and trackSettings.calibratestart is True:
...
class Buttons:
    ...
    def setLat(self):
        trackSettings.Lat = self.latVal.get()

    def setLon(self):
        trackSettings.Lon = self.lonVal.get()

    def setComPort(self):
        trackSettings.comPort = self.comNumber.get()

    def setCamera(self):
        trackSettings.camera = self.camNumber.get()
...
## clean up to this point...
    ...
    def start_sat_track(self):
        ...
        if trackSettings.fileSelected is False:
            ...
            self.textbox.see('end')
        if trackSettings.tracking is True and self.collect_images is True and trackSettings.trackingSat is True and \
                trackSettings.fileSelected is True:
            with open(trackSettings.orbitFile) as f:
                ...
                self.observer.lat = str(trackSettings.Lat)
                self.observer.lon = str(trackSettings.Lon)
            ...
            if firstslew is True:
                ...
                if trackSettings.telescopeType == 'LX200':
                    if trackSettings.mountType == 'AltAz':
                        ...
                        targetcoordaz = str(
                            ':Sz ' + str(self.az_d) + '*' + str(self.az_m) + ':' + str(int(self.az_s)) + '#')
                        targetcoordalt = str(
                            ':Sa ' + str(self.alt_d) + '*' + str(self.alt_m) + ':' + str(int(self.alt_s)) + '#')
                        ...
                        self.textbox.insert(END,
                                            str('Az: ' + str(targetcoordaz) + 'Alt: ' + str(targetcoordalt) + '\n'))
                        self.textbox.see('end')
                    if trackSettings.mountType == 'Eq':
                        satra = self.sat.ra
                        ...
                        targetcoordra = str(
                            ':Sr ' + str(self.ra_h) + '*' + str(self.ra_m) + ':' + str(int(self.ra_s)) + '#')
                        targetcoorddec = str(
                            ':Sd ' + str(self.dec_d) + '*' + str(self.dec_m) + ':' + str(int(self.dec_s)) + '#')
                        self.ser.write(str.encode(targetcoordra))
                        self.ser.write(str.encode(targetcoorddec))
                        self.ser.write(str.encode(':MS#'))
                        print(targetcoordra, targetcoorddec)
                        self.textbox.insert(END,
                                            str('RA: ' + str(targetcoordra) + 'Dec: ' + str(targetcoorddec) + '\n'))
                        self.textbox.see('end')
                    time.sleep(1)
                ...
                if trackSettings.telescopeType == 'ASCOM':
                    ...
                    if trackSettings.mountType == 'AltAz' and trackSettings.trackingSat is True:
                        ...
                        self.textbox.insert(END, str('Target Az: ' + str(self.radaz) + ' Target Alt: ' + str(
                            self.radalt) + '\n'))
                        self.textbox.see('end')
                    ...
                    if trackSettings.mountType == 'Eq' and trackSettings.trackingSat is True:
                        ...
                        self.textbox.insert(END, str('Target RA: ' + str(self.radra) + 'Target Dec: ' + str(
                            self.raddec) + '\n'))
                        self.textbox.see('end')
                ...
                if trackSettings.telescopeType == 'ASCOM':
                    ...
                    if trackSettings.mountType == 'AltAz' and trackSettings.trackingSat is True:
                        ...
                        if self.dnow > self.dlast:
                            ...
                            objectdistance = math.sqrt((objecthorizontal ** 2) + (objectvertical ** 2) - 2 * (
                                        objectvertical * objecthorizontal * math.cos(math.radians(objectangle))))
                            try:
                                objectalt = 90 - math.degrees(math.acos(
                                    math.cos(math.radians(objectdistance)) * math.cos(
                                        math.radians(90 - currentaltdegrees)) + math.sin(
                                        math.radians(objectdistance)) * math.sin(
                                        math.radians(90 - currentaltdegrees)) * math.cos(math.radians(objectangle))))
                                diffinaz = math.degrees(math.acos((math.cos(math.radians(objectdistance)) - math.cos(
                                    math.radians(90 - currentaltdegrees)) * math.cos(math.radians(90 - objectalt))) / (
                                                                              math.sin(math.radians(
                                                                                  90 - currentaltdegrees)) * math.sin(
                                                                          math.radians(90 - objectalt)))))
                                ...
                                self.textbox.insert(END, str('Delta Az: ' + str(azdiff) + ' Delta Alt: ' + str(
                                    altdiff) + '\n'))
                                self.textbox.see('end')
                                self.tel.MoveAxis(0, azrate)
                                self.tel.MoveAxis(1, altrate)
                    ...
                    if trackSettings.mountType == 'Eq' and trackSettings.trackingSat is True:
                        ...
                        if self.dnow > self.dlast:
                            ...
                            objectdistance = math.sqrt((objecthorizontal ** 2) + (objectvertical ** 2) - 2 * (
                                        objectvertical * objecthorizontal * math.cos(math.radians(objectangle))))
                            try:
                                objectdec = 90 - math.degrees(math.acos(
                                    math.cos(math.radians(objectdistance)) * math.cos(
                                        math.radians(90 - currentdecdegrees)) + math.sin(
                                        math.radians(objectdistance)) * math.sin(
                                        math.radians(90 - currentdecdegrees)) * math.cos(math.radians(objectangle))))
                                diffinra = math.degrees(math.acos((math.cos(math.radians(objectdistance)) - math.cos(
                                    math.radians(90 - currentdecdegrees)) * math.cos(math.radians(90 - objectdec))) / (
                                                                              math.sin(math.radians(
                                                                                  90 - currentdecdegrees)) * math.sin(
                                                                          math.radians(90 - objectdec)))))
                                if math.fabs(objectangle2) > 90:
                                    diffinra = -1 * diffinra
                    ...
                    if trackSettings.mountType == 'AltAz':
                        ...
                        if self.dnow > self.dlast:
                            ...
                            try:
                                objectalt = 90 - math.degrees(math.acos(
                                    math.cos(math.radians(objectdistance)) * math.cos(
                                        math.radians(90 - self.telalt)) + math.sin(
                                        math.radians(objectdistance)) * math.sin(
                                        math.radians(90 - self.telalt)) * math.cos(math.radians(objectangle))))
                                diffinaz = math.degrees(math.acos((math.cos(math.radians(objectdistance)) - math.cos(
                                    math.radians(90 - self.telalt)) * math.cos(math.radians(90 - objectalt))) / (
                                                                              math.sin(math.radians(
                                                                                  90 - self.telalt)) * math.sin(
                                                                          math.radians(90 - objectalt)))))
                                if math.fabs(objectangle2) > 90:
                                    diffinaz = -1 * diffinaz
                    ...
                    if trackSettings.mountType == 'EQ':
                        ...
                        if self.dnow > self.dlast:
                            ...
                            try:
                                objectalt = 90 - math.degrees(math.acos(
                                    math.cos(math.radians(objectdistance)) * math.cos(
                                        math.radians(90 - self.teldec)) + math.sin(
                                        math.radians(objectdistance)) * math.sin(
                                        math.radians(90 - self.teldec)) * math.cos(math.radians(objectangle))))
                                diffinaz = math.degrees(math.acos((math.cos(math.radians(objectdistance)) - math.cos(
                                    math.radians(90 - self.teldec)) * math.cos(math.radians(90 - objectalt))) / (
                                                                              math.sin(math.radians(
                                                                                  90 - self.teldec)) * math.sin(
                                                                          math.radians(90 - objectalt)))))
                                if math.fabs(objectangle2) > 90:
                                    diffinra = -1 * diffinra
                                decdiff = math.radians(objectdec) - currentdec
    ...
    def set_calibration(self):
        ...
        if trackSettings.tracking is True and self.collect_images is True and trackSettings.objectfollow is True and trackSettings.calibratestart is True:
            trackSettings.imageScale = self.controller.setCalibration()
            print(trackSettings.imageScale, ' degrees per pixel.')
            self.textbox.insert(END, str('Image scale: ' + str(trackSettings.imageScale) + ' degrees per pixel.\n'))
            self.textbox.see('end')
            trackSettings.calibratestart = False

    ## below is all for set_img_collect
    ...
    # turns on object follow. Primarily sets up the parameters needed to track something in the images.
    #   Seems like a lot of this is reduntant with the image processing code.
    #  creates: self.trackimg that will become a new display of the portion of the image defined by self.mousebox
    #  sets: trackSettings.minbright -- the median between the clicked pixel and the brightest pixes in the ROI
    def left_click(self, event):
        if trackSettings.setcenter is True:
            ...
            # This is a UI element used to show the newly tracked object.
            self.trackimg = Label(self.topframe, bg="black")
            #todo -- do we really need this here!!! I really don't think it does anything
            self.imgtk = self.img.copy()

            #todo -- I think this is a bit of a waste of a global variable.  Basically all of this is local just to
            #     -- get the min brigthness in gray scale land instead of RGB
            ...
            # this is an (alias??? or copy???) of the portion of the image that is bounded by self.mousebox
            ...
            local_clickpixel = self.img[trackSettings.mousecoords[1], trackSettings.mousecoords[0]]
            ...
            for y in range(0, roiheight):
                for x in range(0, roiwidth):
                    ...
                    if pixel > pixellast:
                        pixellast = pixel
            trackSettings.minbright = ((pixellast - local_clickpixel) / 2) + local_clickpixel
            # self.entryBright.delete(0, END)
            # self.entryBright.insert(0, trackSettings.minbright)

            trackSettings.objectfollow = True

    # turns off object follow.
    ...
    # input: self.collection_images
    # modifies: self.img, self.imgtk
    def prepare_img_for_tkinter(self):
        if self.collect_images is True:
            ...
            if ret is True:
                ...
                self.mousebox = [(int(trackSettings.mousecoords[0] - (trackSettings.boxSize / 2)),
                ...
                local_b, local_g, local_r = cv2.split(self.imgtk)
                ...
                self.displayimg.config(image=local_tkimg)
                self.displayimg.img = local_tkimg
